...
bool loadGoBridge() {
    ...
    if (startFunc == nullptr || stopFunc == nullptr || networkChangedFunc == nullptr) {
        logError("required tg-tunnel-go AmneziaWG symbols are missing");
        dlclose(goHandle);
        goHandle = nullptr;
        startFunc = nullptr;
        stopFunc = nullptr;
...
Java_org_telegram_messenger_TunnelManager_nativeAmneziaWGStart(JNIEnv *env, jclass, jstring userspaceConfig, jobjectArray localAddresses, jobjectArray dnsServers, jint mtu) {
    if (!loadGoBridge()) {
        return -100;
    }
