...
build_dir="${TG_WG_BUILD_DIR:-"$script_dir/build"}"
go_version="${TG_WG_GO_VERSION:-1.24.3}"
...
cd "$script_dir"
"$go_bin" build -tags linux -trimpath -buildvcs=false -buildmode c-shared -ldflags "-extldflags=-Wl,-soname,libtg-wg-go.so" -o "$output_so" .
